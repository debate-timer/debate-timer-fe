...
| 테스트 | 입력 | 예상 출력 |
|---|---|---|
| 팀명만 있을 때 첫 번째 줄에 팀명이 표시된다 | `teamName = '찬성'`, `speaker = null` | 팀명 텍스트 화면에 표시, 토론자 줄 미표시 |
| 토론자만 있을 때 두 번째 줄에 토론자 정보가 표시된다 | `teamName = null`, `speaker = '발언자 1'` | 토론자 텍스트 화면에 표시, 팀명 줄 미표시 |
| 팀명과 토론자 모두 있을 때 각각 독립된 줄로 표시된다 | `teamName = '찬성'`, `speaker = '발언자 1'` | 팀명과 토론자가 각각 별도 DOM 요소로 렌더링 |
