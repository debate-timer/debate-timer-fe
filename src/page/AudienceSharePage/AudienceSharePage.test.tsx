...
describe('AudienceSharePage', () => {
  ...
  describe('남은 시간에 따른 배경색', () => {
    const renderNormalTimer = async (
      singleTime: number,
      isRunning: boolean,
    ) => {
      vi.useFakeTimers();
    ...
    it.each([
      ...
      '실행 중인 일반 타이머의 남은 시간이 %i초면 배경 상태는 %s이다',
      async (singleTime, expected) => {
        await renderNormalTimer(singleTime, true);

        expect(screen.getByTestId('audience-content')).toHaveAttribute(
          'data-timer-bg',
          expected,
        );
      },
    ...
    it('정지 중인 타이머는 남은 시간이 적어도 기본 배경이다', async () => {
      await renderNormalTimer(5, false);
    ...
    it('카운트다운이 기준을 지나면 배경색이 바뀐다', async () => {
      await renderNormalTimer(12, true);
      expect(screen.getByTestId('audience-content')).toHaveAttribute(
        'data-timer-bg',
        'warning',
      );
    ...
    it('사회자 연결이 끊기면 기본 배경으로 돌아간다', async () => {
      vi.useFakeTimers();
      mockUseAudienceShareState.mockReturnValue({
    ...
    it('자유토론은 현재 발언 팀의 남은 시간으로 배경색을 정한다', async () => {
      vi.useFakeTimers();
      mockUseAudienceShareState.mockReturnValue({
        chairmanPresence: 'present',
        status: 'displaying',
